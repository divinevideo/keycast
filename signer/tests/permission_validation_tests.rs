...
/// Test context with isolated tenant and team for parallel test execution
struct TestContext {
    pool: PgPool,
    tenant_id: i64,
    team_id: i32,
...
impl TestContext {
    /// Create a new isolated test context with random tenant and team
    async fn new() -> Self {
        ...
        // Create a new tenant with unique domain and get its ID
        ...
        .bind(format!("Test Tenant {}", unique_id))
        .fetch_one(&pool)
        .await
        .expect("Failed to create tenant");

        // Create team and get its actual ID
        let team_id: i32 = sqlx::query_scalar(
            "INSERT INTO teams (name, tenant_id, created_at, updated_at)
             VALUES ($1, $2, NOW(), NOW())
             RETURNING id"
        )
        .bind(format!("Test Team {}", Uuid::new_v4()))
        .bind(tenant_id)
        .fetch_one(&pool)
        .await
        ...
        Self { pool, tenant_id, team_id, key_manager }
    }

    /// Create policy with specified permissions using this context's tenant/team
    async fn create_policy_with_permissions(
        &self,
        permission_configs: Vec<(&str, serde_json::Value)>,
    ) -> i32 {
        // Create policy using the actual team_id from this context
...
async fn create_test_authorization(
    ...
    let encrypted_secret = ctx.key_manager.encrypt(&user_secret).await
    ...
    let encrypted_bunker_secret = ctx.key_manager.encrypt(&bunker_secret).await
    ...
    // Load authorization
    let auth = Authorization::find(&ctx.pool, ctx.tenant_id, auth_id).await
...
async fn create_oauth_authorization(
    ...
    .bind(format!("Test App {}", Uuid::new_v4()))
    ...
    let oauth_auth = OAuthAuthorization::find(&ctx.pool, ctx.tenant_id, oauth_id).await
...
async fn test_1_no_policy_allows_all() {
    ...
    let policy_id = ctx.create_policy_with_permissions(vec![]).await;

    let (auth, bunker_keys, user_keys) = create_test_authorization(&ctx, policy_id).await;
...
async fn test_2_allowed_kinds_permits_matching_kind() {
    ...
    let policy_id = ctx.create_policy_with_permissions(
...
async fn test_7_oauth_no_policy_allows_all() {
    ...
    let (oauth_auth, user_keys) = create_oauth_authorization(&ctx, None).await;
...
async fn test_8_oauth_with_policy_enforces_restrictions() {
    ...
    let (oauth_auth, user_keys) = create_oauth_authorization(&ctx, Some(policy_id)).await;
