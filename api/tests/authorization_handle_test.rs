...
async fn test_authorization_handle_can_be_stored_and_retrieved() {
    ...
    let auth_id: i32 = sqlx::query_scalar(
        "INSERT INTO oauth_authorizations
         (user_pubkey, redirect_origin, client_id, bunker_public_key, secret, relays, tenant_id, authorization_handle, created_at, updated_at)
         VALUES ($1, $2, 'Test App', $3, 'secret', '[]', 1, $4, NOW(), NOW())
         RETURNING id",
    )
    .bind(&user_pubkey)
...
async fn test_authorization_handle_unique_index_on_active() {
    ...
    sqlx::query(
        "INSERT INTO oauth_authorizations
         (user_pubkey, redirect_origin, client_id, bunker_public_key, secret, relays, tenant_id, authorization_handle, created_at, updated_at)
         VALUES ($1, $2, 'Test App', $3, 'secret1', '[]', 1, $4, NOW(), NOW())",
    )
    .bind(&user_pubkey)
    ...
    let result = sqlx::query(
        "INSERT INTO oauth_authorizations
         (user_pubkey, redirect_origin, client_id, bunker_public_key, secret, relays, tenant_id, authorization_handle, created_at, updated_at)
         VALUES ($1, $2, 'Test App', $3, 'secret2', '[]', 1, $4, NOW(), NOW())",
    )
    .bind(&user_pubkey)
...
/// Returns the authorization ID if the handle is valid, active, and not expired
...
async fn test_expired_handle_requires_consent() {
    ...
    sqlx::query(
        "INSERT INTO oauth_authorizations
         (user_pubkey, redirect_origin, client_id, bunker_public_key, secret, relays, tenant_id, authorization_handle, expires_at, created_at, updated_at)
         VALUES ($1, $2, 'Test App', $3, 'secret', '[]', 1, $4, $5, NOW(), NOW())",
    )
    .bind(&user_pubkey)
    .bind(&redirect_origin)
    .bind(bunker_keys.public_key().to_hex())
...
async fn test_handle_lookup_ignores_null_handles() {
    ...
    sqlx::query(
        "INSERT INTO oauth_authorizations
         ...
         VALUES ($1, $2, 'Test App', $3, 'secret', '[]', 1, NOW(), NOW())",
    )
    .bind(&user_pubkey)
    .bind(&redirect_origin)
    .bind(bunker_keys.public_key().to_hex())
