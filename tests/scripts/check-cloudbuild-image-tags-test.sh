...
build_image='us-central1-docker.pkg.dev/${PROJECT_ID}/docker/keycast:${COMMIT_SHA}-${BUILD_ID}'
...
require_text "$cloudbuild" "- '$build_image'"
require_text "$cloudbuild" "- '$latest_image'"
require_text "$cloudbuild" "- '--image=$build_image'"
...
bun test "$repo_root/tests/scripts/cloudbuild-image-tags.test.ts"
