...
2. Builds the Docker image and tags it as both `us-central1-docker.pkg.dev/${PROJECT_ID}/docker/keycast:${COMMIT_SHA}-${BUILD_ID}` and `:latest`.
3. Pushes both tags. Each build has its own commit-and-build tag; `:latest` remains a convenience pointer. Rebuilding the same commit gets a new Cloud Build ID, preserving earlier build tags for rollback.
...
5. Deploys the Cloud Run service `keycast` from its unique commit-and-build tag.
6. Runs smoke checks:
   - `GET /healthz/ready`
   - CORS preflight for `/api/auth/register`
   - CORS preflight for `/api/headless/login`

The deploy commands derive `COMMIT_SHA` from `git rev-parse HEAD`; Cloud Build supplies its unique `BUILD_ID`. The preflight rejects modified tracked content and untracked upload paths so the commit portion describes the submitted source. Build tags are never reused by this pipeline, including concurrent builds of one commit. This does not enable registry-wide tag immutability: `:latest` remains mutable, and registry writers must not manually reassign historical build tags. A recorded image digest is also suitable for an exact-artifact rollback.
...
|---------|-------|
| Service | `keycast` |
| Image | `us-central1-docker.pkg.dev/openvine-co/docker/keycast:<full-commit-sha>-<build-id>` (`:latest` also points to the most recently built image) |
| Region | `us-central1` |
| Port | `3000` |
...
An earlier build can also be redeployed by its recorded commit-and-build tag. Use the full image reference from that Cloud Run revision or Cloud Build record; a commit may have multiple builds with different dependency versions:

```bash
gcloud run services update keycast \
  --image=us-central1-docker.pkg.dev/openvine-co/docker/keycast:<full-commit-sha>-<build-id> \
  --region=us-central1 \
  --project=openvine-co
```
